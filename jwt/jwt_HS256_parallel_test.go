...
func BenchmarkJWT_Create_HS256_String_Empty_Parallel(b *testing.B) {
...
func BenchmarkJWT_Create_HS256_Bytes_Empty_Parallel(b *testing.B) {
	for isl := range stublist {
		taskKey := fmt.Sprintf("%s", stublist[isl].name)
		b.Run(taskKey, func(bs *testing.B) {
			b.ResetTimer()
...
func BenchmarkJWT_Create_HS256_String_Filled_Parallel(b *testing.B) {
	for isl := range stublist {
		taskKey := fmt.Sprintf("%s", stublist[isl].name)
		b.Run(taskKey, func(bs *testing.B) {
			b.ResetTimer()
...
func BenchmarkJWT_Create_HS256_Bytes_Filled_Parallel(b *testing.B) {
	for isl := range stublist {
		taskKey := fmt.Sprintf("%s", stublist[isl].name)
		b.Run(taskKey, func(bs *testing.B) {
			b.ResetTimer()
