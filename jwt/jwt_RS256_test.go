...
func BenchmarkJWT_Create_RS256_String_Empty(b *testing.B) {
...
func BenchmarkJWT_Create_RS256_Bytes_Empty(b *testing.B) {
	for isl := range stublist {
		taskKey := fmt.Sprintf("%s", stublist[isl].name)
		b.Run(taskKey, func(bs *testing.B) {
			bs.ResetTimer()
...
func BenchmarkJWT_Create_RS256_String_Filled(b *testing.B) {
	for isl := range stublist {
		taskKey := fmt.Sprintf("%s", stublist[isl].name)
		b.Run(taskKey, func(bs *testing.B) {
			bs.ResetTimer()
...
func BenchmarkJWT_Create_RS256_Bytes_Filled(b *testing.B) {
	for isl := range stublist {
		taskKey := fmt.Sprintf("%s", stublist[isl].name)
		b.Run(taskKey, func(bs *testing.B) {
			bs.ResetTimer()
